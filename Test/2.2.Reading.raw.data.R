...
genotype.rows <- rownames(genotype)
genotype.rows <- as.numeric(genotype.rows)

# Check for genotype length
length(genotype.rows)
print(genotype)
head(genotype)
...
# Subsetting length of phenotype by length of genotype 
phenotype <- phenotype[phenotype$id %in% genotype.rows,]

phenotype$id[is.na(phenotype$id)] <- FALSE   #removing NAs from the sample

# Check for phenotype length
length(phenotype$id)
...
save(genotype.s, geno.bim, phenotype, file = working.data.fname(2.2))

# On to script 2.3.Data.filtering.R 
