...
print(ncol(genotype.s)-sum(fil.genotype)) # 67059 SNPs will be removed
...
print(genotype.s) #25675 SNPs are remaining
...
print(nrow(genotype.s)-sum(fil.sample))  # 472 subjects removed 
...
cat(length(related.samples), "similar samples removed due to 
    correlation coefficient >=", kin.cutoff, "\n") # 164 similar samples removed
print(genotype.s) # 514 subject remain
...
snpsum.col.cont <- col.summary(genotype.s)
# Filter based on HWE
...
genotype.s <- genotype.s[,HWE.use] #25318 SNPs remain
...
save(genotype.s, geno.bim, genofile, phenotype, file = working.data.fname(2.3))

# on to script 2.4.Data.generation.R
