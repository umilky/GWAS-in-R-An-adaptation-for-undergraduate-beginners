...
# Load data created in previous script, 3.4.Data.filtering.R
load(working.data.fname(3.4))
...
# 229* imputation rules remain after uncertain imputation were removed

rules <- rules[imputation.maf(rules) >= minor]
cat(length(rules), "imputation rules remain after MAF filtering\n")
# 229* imputation rules remain after MAF filtering
...
save(genotype.s, geno.bim, genofile, phenotype, pcs, support, imputed,  
     file = working.data.fname(3.5))


# On to script 3.6.Data.analysis.R
