...
# Load data created in previous script, 3.3.Reading.raw.data.R
load(working.data.fname(3.3))
...
# * values will vary slightly from reference due to random nature of simulation
...
call <- 0.90 # SNPs with 10% missing data is retained after filtering.
minor.f <- 0.01 
...
print(ncol(genotype.s)-sum(fil.genotype)) # 4800* SNPs will be removed
...
print(genotype.s) #5201* SNPs are remaining
...
print(nrow(genotype.s)-sum(fil.sample))  # 41* subjects removed 
...
phenotype <- phenotype[!(phenotype$id %in% related.samples),]

genosample.ids <- rownames(genotype.s)

cat(length(related.samples), "similar samples removed due to 
    correlation coefficient >=", kin.cutoff, "\n")
print(genotype.s) # except all 960* subject remain
...
# disease presence.
d.pres.controls <- phenotype[phenotype$d.pres==0, 'id' ]
snpsum.col.cont <- col.summary(genotype.s[d.pres.controls,])

# Filter based on HWE
HWE.use <- with(snpsum.col.cont, !is.na(z.HWE) & 
                  ( abs(z.HWE) < abs( qnorm(HWE.thres/2))))
...
genotype.s <- genotype.s[,HWE.use] #5201* SNPs remain
...
save(genotype.s, geno.bim, genofile, phenotype, file = working.data.fname(3.4))

# on to script 3.5.Data.generation.R
